		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="采购人员" prop="procurement_personnel">
												<el-input id="procurement_personnel" v-model="form_sub['procurement_personnel']" placeholder="请输入采购人员"
							  v-if="user_group === '管理员' || (form_sub['purchasing_users_id'] && $check_field('set','procurement_personnel')) || (!form_sub['purchasing_users_id'] && $check_field('add','procurement_personnel'))"></el-input>
					<div v-else-if="$check_field('get','procurement_personnel')">{{form_sub['procurement_personnel']}}</div>
											</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["purchasing_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?purchasing_users_id=" + form_sub["purchasing_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/purchasing_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/purchasing_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/purchasing_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/purchasing_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/purchasing_users/view','get');
