		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="供应商编号" prop="supplier_number">
												<el-input id="supplier_number" v-model="form_sub['supplier_number']" placeholder="请输入供应商编号"
							  v-if="user_group === '管理员' || (form_sub['supplier_id'] && $check_field('set','supplier_number')) || (!form_sub['supplier_id'] && $check_field('add','supplier_number'))"></el-input>
					<div v-else-if="$check_field('get','supplier_number')">{{form_sub['supplier_number']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="供应商名称" prop="supplier_name">
												<el-input id="supplier_name" v-model="form_sub['supplier_name']" placeholder="请输入供应商名称"
							  v-if="user_group === '管理员' || (form_sub['supplier_id'] && $check_field('set','supplier_name')) || (!form_sub['supplier_id'] && $check_field('add','supplier_name'))"></el-input>
					<div v-else-if="$check_field('get','supplier_name')">{{form_sub['supplier_name']}}</div>
											</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["supplier_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?supplier_id=" + form_sub["supplier_id"],form_sub,(res)=>{
