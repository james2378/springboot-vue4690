	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="商品图" prop="product_map">
						...
						:show-file-list="false" v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','product_map')) || (!form['purchasing_management_id'] && $check_field('add','product_map'))">
				...
				<el-form-item label="采购单号" prop="purchase_order_number">
							  ...
							  v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','purchase_order_number')) || (!form['purchasing_management_id'] && $check_field('add','purchase_order_number'))" :disabled="true"></el-input>
				...
				<el-form-item label="商品名称" prop="product_name">
							  ...
							  v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','product_name')) || (!form['purchasing_management_id'] && $check_field('add','product_name'))" :disabled="disabledObj['product_name_isDisabled']"></el-input>
				...
				<el-form-item label="商品类型" prop="product_type">
							  ...
							  v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','product_type')) || (!form['purchasing_management_id'] && $check_field('add','product_type'))" :disabled="disabledObj['product_type_isDisabled']"></el-input>
				...
				<el-form-item label="品牌" prop="brand">
							  ...
							  v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','brand')) || (!form['purchasing_management_id'] && $check_field('add','brand'))" :disabled="disabledObj['brand_isDisabled']"></el-input>
				...
				<el-form-item label="商品规格" prop="commodity_specifications">
							  ...
							  v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','commodity_specifications')) || (!form['purchasing_management_id'] && $check_field('add','commodity_specifications'))" :disabled="disabledObj['commodity_specifications_isDisabled']"></el-input>
				...
				<el-form-item label="供应商编号" prop="supplier_number">
													<el-select v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','supplier_number')) || (!form['purchasing_management_id'] && $check_field('add','supplier_number'))" id="supplier_number" v-model="form['supplier_number']" :disabled="disabledObj['supplier_number_isDisabled']">
				...
				<el-form-item label="供应商名称" prop="supplier_name">
							  ...
							  v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','supplier_name')) || (!form['purchasing_management_id'] && $check_field('add','supplier_name'))" :disabled="disabledObj['supplier_name_isDisabled']"></el-input>
				...
				<el-form-item label="采购数量" prop="purchase_quantity">
						...
						v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','purchase_quantity')) || (!form['purchasing_management_id'] && $check_field('add','purchase_quantity'))" :disabled="disabledObj['purchase_quantity_isDisabled']"></el-input-number>
				...
				<el-form-item label="采购人员" prop="procurement_personnel">
							...
							{{ get_user_session_procurement_personnel(form['procurement_personnel']) }}
							<!--<el-input id="business_name" v-model="form['procurement_personnel']" placeholder="请输入采购人员"-->
							<!--v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','procurement_personnel')) || (!form['purchasing_management_id'] && $check_field('add','procurement_personnel'))" :disabled="disabledObj['procurement_personnel_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','procurement_personnel')">{{form['procurement_personnel']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','procurement_personnel')) || (!form['purchasing_management_id'] && $check_field('add','procurement_personnel'))" id="procurement_personnel" v-model="form['procurement_personnel']" :disabled="disabledObj['procurement_personnel_isDisabled']">
						...
						<el-select v-else id="procurement_personnel" v-model="form['procurement_personnel']" :disabled="disabledObj['procurement_personnel_isDisabled']">
							<el-option v-for="o in list_user_procurement_personnel" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
									   :value="o['user_id']">
							</el-option>
						</el-select>
				...
				<el-form-item label="采购日期" prop="purchase_date">
								<el-date-picker :disabled="disabledObj['purchase_date_isDisabled']" v-if="user_group === '管理员' || (form['purchasing_management_id'] && $check_field('set','purchase_date')) || (!form['purchasing_management_id'] && $check_field('add','purchase_date'))" id="purchase_date"
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/purchasing_management/add?",
				url_set: "~/api/purchasing_management/set?",
				url_get_obj: "~/api/purchasing_management/get_obj?",
				url_upload: "~/api/purchasing_management/upload?",

				query: {
					"purchasing_management_id": 0,
				},
				...
				list_user_procurement_personnel: [],
						// 用户组
				group_user_procurement_personnel: "",
		...
		methods: {
			 ...
			 * 获取采购用户用户组
			 */
			async get_group_user_procurement_personnel() {
							this.form["procurement_personnel"] = this.user.user_id;
							var json = await this.$get("~/api/user_group/get_obj?name=采购用户");
				if(json.result && json.result.obj){
					this.group_user_procurement_personnel = json.result.obj;
				}
				else if(json.error){
					console.error(json.error);
				}
			},
			get_user_session_procurement_personnel(id){
				var _this = this;
				var user_id = {"user_id":id}
				var url = "~/api/"+_this.group_user_procurement_personnel.source_table+"/get_obj?"
					...
					if (res.result && res.result.obj) {
						...
						var arrForm = []
							...
							arrForm.push(key)
												...
												_this.form["procurement_personnel"] = id
									_this.disabledObj['procurement_personnel' + '_isDisabled'] = true
						for (var i=0;i<arr.length;i++){
						  if (arr[i]!=='examine_state' && arr[i]!=='examine_reply') {
							for (var j = 0; j < arrForm.length; j++) {
							  if (arr[i] === arrForm[j]) {
								if (arr[i] !== "procurement_personnel") {
          ...
          this.form["purchase_date"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/purchasing_management/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/purchasing_management/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/purchasing_management/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/purchasing_management/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/purchasing_management/view','get');
		...
		created() {
					...
					this.get_group_user_procurement_personnel();
							},
	}
</script>
